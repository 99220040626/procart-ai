...
// Extracted Clock to prevent global dashboard re-renders every second
const SystemClock = () => {
...
function AdminDashboard() {
    ...
    const [activeTab, setActiveTab] = useState('inventory'); // Defaulted to inventory to match screenshot
    ...
    const COLORS = ['#6366f1', '#10b981', '#f59e0b', '#8b5cf6', '#ef4444', '#0ea5e9', '#14b8a6'];
    const PLACEHOLDER_IMG = 'https://placehold.co/400x400/f8fafc/94a3b8?text=No+Image';
    ...
    return (
        <div className="min-h-screen bg-slate-50 dark:bg-[#0a0a0a] text-slate-900 dark:text-slate-100 py-8 px-4 sm:px-6 lg:px-8 font-sans transition-colors duration-300">
            <div className="max-w-[1400px] mx-auto">
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-sm text-slate-500 dark:text-slate-400">Manage your store operations and inventory.</p>
                        </div>
                    </div>
                    <SystemClock />
                </div>
                ...
                {/* Tabs */}
                <div className="flex overflow-x-auto custom-scrollbar bg-slate-200/50 dark:bg-slate-800/50 p-1.5 rounded-xl mb-8 border border-slate-200 dark:border-slate-700 w-fit max-w-full shadow-sm">
                    {[
                        ...
                        <button 
                            ...
                            className={`relative px-5 py-2.5 text-sm font-semibold rounded-lg whitespace-nowrap transition-all duration-200 ${
                                activeTab === tab.id 
                ...
                {/* Main Content Area */}
                {loading ? (
                    <div className="flex flex-col items-center justify-center py-32 bg-white dark:bg-[#111] rounded-2xl border border-slate-200 dark:border-slate-800">
                    ...
                    <div className="bg-white dark:bg-[#111] rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 overflow-hidden min-h-[500px] transition-all">
                        ...
                        {/* ===================== INVENTORY TAB (Fixing Screenshot Issue) ===================== */}
                        {activeTab === 'inventory' && (
                            <div className="p-6 lg:p-8">
                                <div className="grid grid-cols-1 xl:grid-cols-3 gap-8 mb-10">
                                    ...
                                    <div className="xl:col-span-2 bg-slate-50 dark:bg-[#151515] p-6 rounded-2xl border border-slate-200 dark:border-slate-800">
                                        <div className="flex items-center gap-2 mb-6">
                                            ...
                                            <h3 className="font-semibold text-slate-900 dark:text-white uppercase tracking-wider text-xs">Add New Product</h3>
                                        </div>
                                        <form onSubmit={handleAddProduct} className="grid grid-cols-1 md:grid-cols-2 gap-5">
                                            <div>
                                                <label className="block text-xs font-semibold text-slate-500 dark:text-slate-400 mb-1.5 uppercase tracking-wide">Product Name</label>
                                            ...
                                            <div>
                                                <label className="block text-xs font-semibold text-slate-500 dark:text-slate-400 mb-1.5 uppercase tracking-wide">Price (₹)</label>
                                            ...
                                            <div>
                                                <label className="block text-xs font-semibold text-slate-500 dark:text-slate-400 mb-1.5 uppercase tracking-wide">Units Available</label>
                                            ...
                                            <div className="md:col-span-2">
                                                <label className="block text-xs font-semibold text-slate-500 dark:text-slate-400 mb-1.5 uppercase tracking-wide">Detailed Specifications</label>
                                                <textarea rows="3" value={newProduct.detailedDescription} onChange={e => setNewProduct({...newProduct, detailedDescription: e.target.value})} className="w-full p-2.5 rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-[#1a1a1a] text-slate-900 dark:text-white focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none transition-all text-sm resize-none" placeholder="Enter product features..."></textarea>
                                            </div>

                                            <div className="md:col-span-2 grid grid-cols-1 sm:grid-cols-3 gap-4 pt-4 border-t border-slate-200 dark:border-slate-800">
                                                <div>
                                                    <label className="block text-xs font-semibold text-slate-500 dark:text-slate-400 mb-1.5 uppercase tracking-wide">Primary Image</label>
                                                    <input type="file" accept="image/*" onChange={e => setSelectedImage(e.target.files[0])} className="w-full text-xs text-slate-500 file:mr-3 file:py-2 file:px-4 file:rounded-md file:border-0 file:text-xs file:font-semibold file:bg-slate-200 dark:file:bg-slate-800 file:text-slate-700 dark:file:text-slate-300 hover:file:bg-slate-300 dark:hover:file:bg-slate-700 transition cursor-pointer" />
                                                </div>
                                                <div>
                                                    <label className="block text-xs font-semibold text-slate-500 dark:text-slate-400 mb-1.5 uppercase tracking-wide">Gallery Images</label>
                                                    <input type="file" multiple accept="image/*" onChange={e => setGalleryFiles(e.target.files)} className="w-full text-xs text-slate-500 file:mr-3 file:py-2 file:px-4 file:rounded-md file:border-0 file:text-xs file:font-semibold file:bg-slate-200 dark:file:bg-slate-800 file:text-slate-700 dark:file:text-slate-300 hover:file:bg-slate-300 transition cursor-pointer" />
                                                </div>
                                                <div>
                                                    <label className="block text-xs font-semibold text-indigo-500 dark:text-indigo-400 mb-1.5 uppercase tracking-wide">3D Model (.glb)</label>
                                                    <input type="file" accept=".glb,.gltf" onChange={e => setSelectedModel(e.target.files[0])} className="w-full text-xs text-slate-500 file:mr-3 file:py-2 file:px-4 file:rounded-md file:border-0 file:text-xs file:font-semibold file:bg-indigo-50 dark:file:bg-indigo-900/20 file:text-indigo-700 dark:file:text-indigo-300 hover:file:bg-indigo-100 transition cursor-pointer" />
                                                </div>
                                            </div>
                                            
                                            <button type="submit" className="md:col-span-2 mt-2 bg-indigo-600 hover:bg-indigo-700 text-white font-semibold py-3 px-4 rounded-lg transition shadow-md text-sm uppercase tracking-wide">Publish Product</button>
                                        </form>
                                    </div>
                                    
                                    {/* Bulk Upload Card */}
                                    <div className="bg-gradient-to-br from-indigo-50 to-white dark:from-[#111526] dark:to-[#0a0a0a] p-6 rounded-2xl border border-indigo-100 dark:border-indigo-900/50 flex flex-col justify-between shadow-sm">
                                        <div>
                                            <h3 className="font-semibold text-indigo-900 dark:text-indigo-300 mb-2 uppercase tracking-wider text-xs">Bulk Import Data</h3>
                                            <p className="text-sm text-slate-600 dark:text-slate-400 mb-6 leading-relaxed">Upload an Excel/CSV file to create multiple products at once. Format: Name | Price | Stock | Class</p>
                                        </div>
                                        <form onSubmit={handleExcelUpload} className="bg-white dark:bg-[#151515] p-5 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm">
                                            <input type="file" id="excel-upload-input" accept=".xlsx, .xls, .csv" onChange={e => setExcelFile(e.target.files[0])} className="w-full mb-4 text-xs text-slate-500 file:mr-3 file:py-2 file:px-4 file:rounded-md file:border-0 file:text-xs file:font-semibold file:bg-slate-100 dark:file:bg-slate-800 file:text-slate-700 cursor-pointer transition" />
                                            <button type="submit" disabled={isUploadingExcel} className="w-full bg-slate-900 dark:bg-indigo-600 text-white font-semibold py-3 rounded-lg hover:bg-slate-800 dark:hover:bg-indigo-500 transition shadow-sm text-sm disabled:opacity-50 disabled:cursor-not-allowed uppercase tracking-wide">
                                                {isUploadingExcel ? 'Processing Data...' : 'Upload File'}
                                            </button>
                                        </form>
                                    </div>
                                </div>
                                ...
                                <div className="grid grid-cols-2 md:grid-cols-4 xl:grid-cols-5 gap-6">
                                    {products.map(product => (
                                        <div key={product.id} className="group relative flex flex-col bg-white dark:bg-[#151515] rounded-xl border border-slate-200 dark:border-slate-800 overflow-hidden hover:shadow-xl hover:border-indigo-300 dark:hover:border-indigo-500/50 transition-all duration-300">
                                            <div className="aspect-square bg-slate-50 dark:bg-[#1a1a1a] relative flex items-center justify-center overflow-hidden">
                                                {/* FIXED: Robust Image Fallback added here */}
                                                <img 
                                                ...
                                                {product.modelUrl && <span className="absolute top-2 right-2 bg-white/90 dark:bg-black/80 backdrop-blur shadow-sm text-indigo-600 dark:text-indigo-400 text-[10px] font-bold px-2 py-1 rounded-md border border-indigo-100 dark:border-indigo-900">3D</span>}
                                                
                                                {/* Hover Actions */}
                                                <div className="absolute inset-0 bg-slate-900/40 dark:bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center gap-3 backdrop-blur-[2px]">
                                                    <button onClick={() => setEditingProduct(product)} className="w-10 h-10 bg-white text-slate-900 rounded-full flex items-center justify-center hover:scale-110 transition-transform shadow-lg" aria-label="Edit">
                                                        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"></path></svg>
                                                    </button>
                                                    <button onClick={() => handleDeleteProduct(product.id)} className="w-10 h-10 bg-red-500 text-white rounded-full flex items-center justify-center hover:scale-110 transition-transform shadow-lg" aria-label="Delete">
                                                        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg>
                                                    </button>
                                                </div>
                                            </div>
                                            <div className="p-4 flex flex-col flex-grow border-t border-slate-100 dark:border-slate-800">
                                                <div className="flex justify-between items-center mb-1">
                                                    <span className="text-[10px] font-semibold text-slate-400 uppercase tracking-wider">#{product.id}</span>
                                                    <span className="text-[10px] font-semibold text-indigo-600 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-900/30 px-2 py-0.5 rounded-full">{product.stock} in stock</span>
                                                </div>
                                                <h3 className="font-semibold text-sm text-slate-900 dark:text-slate-100 line-clamp-1 mt-1">{product.name}</h3>
                                                <div className="mt-2">
                        ...
                        {activeTab === 'analytics' && analytics && (
                            <div className="p-6 lg:p-8">
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                                    {[
                                        { label: 'Gross Volume', value: `₹${analytics.totalRevenue.toLocaleString()}`, color: 'text-indigo-600 dark:text-indigo-400' },
                                        { label: 'Total Orders', value: analytics.totalOrders, color: 'text-emerald-600 dark:text-emerald-400' },
                                        { label: 'Active Products', value: analytics.totalProducts, color: 'text-blue-600 dark:text-blue-400' }
                                    ].map((stat, idx) => (
                                        <div key={idx} className="bg-slate-50 dark:bg-[#151515] p-6 rounded-2xl border border-slate-200 dark:border-slate-800 flex flex-col justify-center">
                                            <p className="text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400 mb-2">{stat.label}</p>
                                            <h3 className={`text-4xl font-black tracking-tight ${stat.color}`}>{stat.value}</h3>
                                        </div>
                                    ))}
                                </div>
                                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                                    <div className="bg-slate-50 dark:bg-[#151515] p-6 rounded-2xl border border-slate-200 dark:border-slate-800">
                                        <h3 className="text-sm font-semibold uppercase tracking-wider text-slate-800 dark:text-slate-200 mb-6">Sales Distribution</h3>
                                    ...
                                    <div className="bg-slate-50 dark:bg-[#151515] p-6 rounded-2xl border border-slate-200 dark:border-slate-800">
                                        <h3 className="text-sm font-semibold uppercase tracking-wider text-slate-800 dark:text-slate-200 mb-6">Category Volume</h3>
                                        <div className="h-[300px]">
                                            <ResponsiveContainer width="100%" height="100%">
                        ...
                        {activeTab === 'users' && (
                            <div className="overflow-x-auto">
                                <table className="w-full text-left border-collapse">
                                    <thead>
                                        <tr className="bg-slate-50 dark:bg-[#151515] border-b border-slate-200 dark:border-slate-800 text-xs text-slate-500 font-semibold uppercase tracking-wider">
                                    ...
                                    <tbody className="divide-y divide-slate-100 dark:divide-slate-800/50 text-sm">
                                        {users.map(user => (
                                            <tr key={user.id} className="hover:bg-slate-50 dark:hover:bg-[#1a1a1a] transition-colors">
                                                ...
                                                <td className="p-4 font-semibold text-slate-900 dark:text-slate-100">{user.name}</td>
                                        ...
                                        {orders.map(order => {
                                            ...
                                            return (
                                                <tr key={order.id} onClick={() => setSelectedOrder(order)} className="hover:bg-slate-50 dark:hover:bg-[#1a1a1a] transition-colors group cursor-pointer text-sm">
                                                    <td className="p-4 pl-6 font-mono text-slate-500 text-xs">#{order.id}</td>
                                                    <td className="p-4">
                                                        <div className="flex items-center gap-3">
                                                            <div className="h-10 w-10 rounded-lg border border-slate-200 dark:border-slate-700 overflow-hidden bg-slate-100 dark:bg-slate-800 shrink-0">
                                                                {product?.imageUrl ? <img src={getImageUrl(product.imageUrl)} onError={handleImageError} alt={product.name} className="h-full w-full object-cover"/> : <div className="h-full w-full flex items-center justify-center text-slate-400 text-[10px]">N/A</div>}
                                                            </div>
                                                            <div>
                                                                <div className="font-semibold text-slate-900 dark:text-slate-100 line-clamp-1">{product?.name || 'Deleted Product'}</div>
                                                                <div className="text-xs text-slate-500 mt-0.5 font-medium">Qty: {order.quantity}</div>
                                                            </div>
                                                        </div>
                                                    </td>
                                                    ...
                                                    <td className="p-4">
                                                        <div className="text-slate-900 dark:text-slate-200 line-clamp-1 text-xs font-medium">{order.shippingAddress?.split(',')[0] || 'No Address'}</div>
                                                        <div className="text-[11px] text-slate-500 mt-0.5">{order.phoneNumber || 'No Phone'}</div>
                                                    </td>
                                                    <td className="p-4">
                                                        <span className={`inline-flex px-2.5 py-1 rounded-md text-[10px] font-bold tracking-wider uppercase ${getStatusBadge(order.status)}`}>
                                                            {order.status}
                                                        </span>
                                                    </td>
                                                    <td className="p-4 text-right pr-6 space-x-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                                        <button onClick={(e) => handleUpdateStatus(order.id, 'SHIPPED', e)} className="px-3 py-1.5 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700 rounded-md text-xs font-semibold transition-colors shadow-sm">Ship</button>
                                                        <button onClick={(e) => handleUpdateStatus(order.id, 'DELIVERED', e)} className="px-3 py-1.5 bg-indigo-50 dark:bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 hover:bg-indigo-100 dark:hover:bg-indigo-500/20 rounded-md text-xs font-semibold transition-colors">Complete</button>
                                                    </td>
                                                </tr>
                                            );
                                        })}
                        ...
                        {activeTab === 'qa' && (
                            <div className="p-6 lg:p-8 max-w-4xl mx-auto">
                                ...
                                {unansweredQA.length === 0 ? (
                                    <div className="text-center py-20 text-slate-500 font-medium">All questions answered! 🎉</div>
                                ) : (
                                            ...
                                            return (
                                                <div key={qa.id} className="bg-slate-50 dark:bg-[#151515] p-5 rounded-xl border border-slate-200 dark:border-slate-800 flex flex-col sm:flex-row gap-6">
                                                    <div className="sm:w-1/3">
                                                        <span className="inline-flex text-[10px] font-bold uppercase tracking-wider bg-indigo-100 text-indigo-700 dark:bg-indigo-900/30 dark:text-indigo-400 px-2 py-0.5 rounded mb-2">Product Inquiry</span>
                                                        <h3 className="font-semibold text-sm text-slate-900 dark:text-slate-200 mb-1">{product?.name || "Unknown Product"}</h3>
                                                    ...
                                                    <div className="flex-1 flex flex-col justify-center">
                                                        ...
                                                        <div className="flex gap-2">
                                                            ...
                                                            <button onClick={() => handleReplyQA(qa.id)} className="bg-indigo-600 text-white font-semibold px-5 rounded-lg hover:bg-indigo-700 transition shadow-sm text-sm whitespace-nowrap">Reply</button>
                                                        </div>
                                                    </div>
                                                </div>
                                            );
                        ...
                        {activeTab === 'audit' && (
                            <div className="p-6 lg:p-8">
                                ...
                                {audits.length === 0 ? (
                                    <div className="text-center py-20 text-slate-500 font-medium">No recent activity.</div>
                                ) : (
                                        ...
                                        <div className="divide-y divide-slate-100 dark:divide-slate-800/50">
                                            {audits.map((audit) => (
                                                <div key={audit.id} className="flex items-center gap-4 p-4 hover:bg-slate-50 dark:hover:bg-[#1a1a1a] transition-colors text-sm">
                                                    <span className="text-slate-400 font-mono text-xs w-40 shrink-0">{new Date(audit.timestamp).toLocaleString()}</span>
                                                    <span className={`px-2.5 py-1 rounded-md text-[10px] font-bold tracking-wider uppercase w-24 text-center shrink-0 ${
                                                    ...
                                                    <span className="text-slate-700 dark:text-slate-300 flex-1 font-medium">{audit.details}</span>
                                                </div>
                                            ))}
                                        </div>
                                    </div>
                ...
                {editingProduct && (
                    <div className="fixed inset-0 z-50 bg-slate-900/40 dark:bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 animate-[fadeIn_0.2s_ease-out]">
                        <div className="bg-white dark:bg-[#151515] w-full max-w-2xl rounded-2xl overflow-hidden shadow-2xl border border-slate-200 dark:border-slate-800 flex flex-col max-h-[90vh] animate-[slideUp_0.3s_ease-out]">
                            <div className="px-6 py-5 border-b border-slate-200 dark:border-slate-800 flex justify-between items-center bg-slate-50 dark:bg-[#111]">
                            ...
                            <div className="flex-1 overflow-y-auto p-6 custom-scrollbar">
                                <form id="edit-form" onSubmit={handleSaveEdit} className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                                    ...
                                    <div>
                                        <label className="block text-xs font-semibold text-slate-500 dark:text-slate-400 mb-1.5 uppercase tracking-wide">Stock</label>
                            ...
                            <div className="px-6 py-4 border-t border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-[#111] flex justify-end gap-3">
                                <button onClick={() => setEditingProduct(null)} className="px-5 py-2.5 rounded-lg text-sm font-semibold text-slate-600 dark:text-slate-400 hover:bg-slate-200 dark:hover:bg-slate-800 transition-colors">Cancel</button>
                                <button form="edit-form" type="submit" className="px-5 py-2.5 rounded-lg text-sm font-semibold bg-indigo-600 hover:bg-indigo-700 text-white transition-colors shadow-sm">Save Changes</button>
                            </div>
                        </div>
                    </div>
                )}
                            ...
                            <div className="bg-white dark:bg-[#151515] w-full max-w-5xl rounded-2xl overflow-hidden shadow-2xl border border-slate-200 dark:border-slate-800 flex flex-col max-h-[90vh] animate-[slideUp_0.3s_ease-out]">
                                ...
                                <div className="px-6 py-5 border-b border-slate-200 dark:border-slate-800 flex justify-between items-center bg-white dark:bg-[#151515]">
                                    <div>
                                        ...
                                        <p className="text-sm text-slate-500 mt-1 font-medium">ID: #{currentOrder.id} • {new Date(currentOrder.orderDate || Date.now()).toLocaleString()}</p>
                                    </div>
                                    ...
                                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                                        ...
                                        <div className="space-y-6">
                                            <div className="bg-white dark:bg-[#151515] p-6 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm">
                                                <h3 className="text-xs font-bold text-slate-500 dark:text-slate-400 mb-5 uppercase tracking-wider">Item Summary</h3>
                                                <div className="flex gap-5">
                                                    <div className="w-24 h-24 rounded-xl bg-slate-100 dark:bg-slate-800 overflow-hidden border border-slate-200 dark:border-slate-700 shrink-0">
                                                        {product?.imageUrl ? <img src={getImageUrl(product.imageUrl)} onError={handleImageError} alt={product?.name} className="w-full h-full object-cover"/> : <div className="w-full h-full flex items-center justify-center text-slate-400 text-xs">No Image</div>}
                                                    </div>
                                                    <div className="flex-1 flex flex-col justify-center">
                                                        <h4 className="font-bold text-slate-900 dark:text-white text-lg">{product?.name || 'Unknown Product'}</h4>
                                                        <div className="mt-auto flex justify-between items-center py-2">
                                                            <span className="text-sm font-medium text-slate-500">Qty: {currentOrder.quantity}</span>
                                                            <span className="font-black text-xl text-slate-900 dark:text-white">₹{(currentOrder.price * currentOrder.quantity).toLocaleString()}</span>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                            ...
                                            {(currentOrder.status === 'PENDING' || currentOrder.paymentScreenshot) && (
                                                <div className="bg-white dark:bg-[#151515] p-6 rounded-2xl border border-amber-200 dark:border-amber-900/50 shadow-sm relative">
                                                    <h3 className="text-xs font-bold text-slate-800 dark:text-slate-200 mb-5 uppercase tracking-wider flex items-center gap-2">
                                                    ...
                                                    <div className="bg-slate-50 dark:bg-[#1a1a1a] rounded-xl overflow-hidden border border-dashed border-slate-300 dark:border-slate-700 min-h-[200px] flex items-center justify-center">
                                                        {currentOrder.paymentScreenshot ? (
                                                            <a href={currentOrder.paymentScreenshot} target="_blank" rel="noreferrer" className="w-full h-full p-2 flex justify-center">
                                                                <img src={currentOrder.paymentScreenshot} onError={handleImageError} alt="Payment Proof" className="max-w-full h-auto object-contain max-h-64 rounded-lg hover:opacity-90 transition" />
                                                            </a>
                                                        ) : (
                                                            <p className="text-sm font-medium text-slate-500 text-center py-10">No payment receipt attached.</p>
                                                        )}
                                                    </div>
                                                </div>
                                            )}
                                        ...
                                        <div className="space-y-6">
                                            <div className="bg-white dark:bg-[#151515] p-6 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm">
                                                <h3 className="text-xs font-bold text-slate-500 dark:text-slate-400 mb-5 uppercase tracking-wider">Customer Information</h3>
                                                <div className="space-y-4">
                                                    <div className="grid grid-cols-3 text-sm border-b border-slate-100 dark:border-slate-800 pb-3">
                                                        <span className="text-slate-500 font-medium">Name</span>
                                                        <span className="col-span-2 font-bold text-slate-900 dark:text-white">{buyer?.name || 'Guest User'}</span>
                                                    </div>
                                                    <div className="grid grid-cols-3 text-sm border-b border-slate-100 dark:border-slate-800 pb-3">
                                                        <span className="text-slate-500 font-medium">Email</span>
                                                        <span className="col-span-2 font-bold text-slate-900 dark:text-white">{buyer?.email || 'N/A'}</span>
                                                    </div>
                                                    <div className="grid grid-cols-3 text-sm">
                                                        <span className="text-slate-500 font-medium">Phone</span>
                                                        <span className="col-span-2 font-bold text-slate-900 dark:text-white">{currentOrder.phoneNumber || 'N/A'}</span>
                                                    </div>
                                                </div>
                                            </div>
                                            ...
                                            <div className="bg-white dark:bg-[#151515] p-6 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm">
                                                <div className="flex justify-between items-center mb-5">
                                                    <h3 className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Logistics & Shipping</h3>
                                                    <span className={`px-3 py-1 rounded-full text-[10px] uppercase font-bold tracking-wider ${getStatusBadge(currentOrder.status)}`}>
                                                        {currentOrder.status}
                                                    </span>
                                                </div>
                                ...
                                <div className="px-6 py-4 border-t border-slate-200 dark:border-slate-800 bg-white dark:bg-[#151515] flex flex-wrap justify-between items-center gap-4">
                                    <button onClick={() => handleUpdateTracking(currentOrder.id)} className="px-4 py-2.5 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 rounded-lg text-sm font-bold transition-colors">
                                        Update Tracking
                                    </button>
                                    
                                    <div className="flex gap-3">
                                        {currentOrder.status === 'PENDING' && (
                                            <button onClick={() => handleUpdateStatus(currentOrder.id, 'SUCCESS')} className="px-5 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg text-sm font-bold transition-colors shadow-sm">
                                                Verify & Approve
                                            </button>
                                        )}
                                        {['SUCCESS', 'PENDING'].includes(currentOrder.status) && (
                                            <button onClick={() => handleUpdateStatus(currentOrder.id, 'SHIPPED')} className="px-5 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg text-sm font-bold transition-colors shadow-sm">
                                                Mark as Shipped
                                            </button>
                                        )}
                                        {currentOrder.status === 'SHIPPED' && (
                                            <button onClick={() => handleUpdateStatus(currentOrder.id, 'DELIVERED')} className="px-5 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg text-sm font-bold transition-colors shadow-sm">
                                                Mark as Delivered
                                            </button>
                                        )}
                                    </div>
